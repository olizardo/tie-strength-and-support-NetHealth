...
To test how tie strength indicators and role relations independently predict membership in each of the inductively identified support configurations while accounting for the nesting of multiple ties within respondents, we formulated multilevel generalized linear mixed models (GLMMs) with a logit link function using the \texttt{lme4} package \citep{bates2015fitting} in \textsf{R} \citep{rcore2026r}. Model visualizations were created using \texttt{ggplot2} \citep{wickham2016ggplot2}. Rather than predicting disaggregated individual support items---which would render the latent class typologizing redundant---the dependent variables in this confirmatory stage are the four distinct relational regimes identified by the LCA: Comprehensive Support, Casual Companionship, Instrumental Support, and Peripheral Support.
